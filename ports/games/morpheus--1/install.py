...
class Main(Installer):
    def __init__(self, app_desc: AppDesc):
        ...
        if app_desc.distro.format == "3CD":
            ...
            w.add_cdrom(SECOND_CD_DRIVE_LETTER, dst_folder / SECOND_CD_DRIVE_LETTER)
            w.add_cdrom(THIRD_CD_DRIVE_LETTER, dst_folder / THIRD_CD_DRIVE_LETTER)
            # install into D:\app, ignore QT install errors, there are few
            w.run(FIRST_CD_DRIVE / "Setup.exe", virtual_desktop=VirtualDesktopResolution.RES_640_480)
            w.gen_run_script(
                app_exec=APP_EXEC,
            )
        else:
